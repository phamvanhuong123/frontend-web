import { Col, Divider, Form, Radio, Row, message, notification, Input } from 'antd';
...
import { callPlaceOrder } from '../../../services/axios.order';
...
const Payment: React.FC<PaymentProps> = ({ setCurrentStep }) => {
    ...
    useEffect(() => {
        if (carts && carts.length > 0) {
            const sum = carts.reduce((acc: number, item: any) => acc + item.quantity * item.detail.price, 0);
            setTotalPrice(sum);
        } else {
            setTotalPrice(0);
        }
